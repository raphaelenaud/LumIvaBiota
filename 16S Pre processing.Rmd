...
# Selection of bacterial ASVs
```{r, echo=FALSE}
phylo = subset_taxa(phylo, Kingdom=="Bacteria")
phylo

#Removal of phylum with a single taxa
phylo <- subset_taxa(phylo, !is.na(Phylum) & !Phylum %in% c("","Bdellovibrionota","Chloroflexi","Deinococcota","<NA>","NA"))
...
# ASV present in less than 3 samples
condition <- function(x) { sum(x > 0) >= 2 }
taxaToKeep <- filter_taxa(phylo, condition)
phylo <- prune_taxa(taxaToKeep, phylo)
